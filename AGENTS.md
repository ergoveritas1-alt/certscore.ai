...
Multilingual privacy evidence coverage supports `en`, `de`, `fr`, `es`, `it`, `nl`, and `pl`. Privacy surfaces, consent controls, and GDPR Transparency topics must use canonical classifiers/registries; localized text should normalize to typed evidence before findings. Do not add display-layer phrase lists, raw text shortcuts, or consent-clicking runtime behavior for multilingual coverage.
