...
The product owner approved one bounded diagnostic exception on August 13, 2026 for the 35-domain adjudicated privacy-policy miss cohort recorded in `artifacts/policy-capture-diagnosis-baseline-834-20260813/policy-miss-live-exception-manifest.json`. This exception permits one policy-only browser discovery attempt per listed target before the ordinary 28-day cooldown expires. It is not an acceptance sample and expires after the single run or at 2026-08-14T23:59:59Z, whichever occurs first. The run must use the exact manifest, no consent interaction, sequential execution, no automatic retry, repository contact holds, immediate stop for each blocked/rate-limited/captcha target, an idempotent run key, and central contact persistence. It does not create a general cooldown waiver or authorize any unlisted domain.
