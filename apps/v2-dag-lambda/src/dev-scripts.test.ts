...
test("Dockerfile uses Playwright image and Lambda runtime interface client", async () => {
  const dockerfile = await readRepoFile("apps/v2-dag-lambda/Dockerfile");

  assert.match(dockerfile, /mcr\.microsoft\.com\/playwright:v1\.51\.1-noble/);
  assert.match(dockerfile, /aws-lambda-ric/);
