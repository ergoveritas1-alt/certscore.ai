...
import { VALIDATION_ALLOWED_EMAIL } from "../validation/auth";
...
export async function submitCredentialsAction(
  ...
  try {
    ...
    if (mode === "create_account") {
      return {
        accountRecovery: null,
        error: "Validation sign-in only allows the existing bmasek@gmail.com account.",
        fieldErrors: {},
        mode
      };
    }

    if (normalizedEmail !== VALIDATION_ALLOWED_EMAIL) {
      return {
        accountRecovery: null,
        error: "Only the bmasek@gmail.com account can access Validation Ops.",
        fieldErrors: {},
        mode
      };
    }
