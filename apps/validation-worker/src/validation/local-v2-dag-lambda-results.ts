...
type LambdaResultMessage = {
  ...
  laneTimingSummary?: LambdaLaneTimingSummary;
...
function parseLambdaLaneTimingSummary(value: unknown): LambdaLaneTimingSummary | undefined {
  const record = asRecord(value);
  ...
  const lanes = record.lanes.flatMap((value) => {
    ...
    return [{
      coordinatorElapsedMs: nullableInteger(laneRecord.coordinatorElapsedMs, true),
      evidenceJoined: laneRecord.evidenceJoined === true,
      invocationStartedAt: nullableDate("invocationStartedAt"),
      lane: lane as LambdaLaneTimingSummary["lanes"][number]["lane"],
      outcome: outcome as LambdaLaneTimingSummary["lanes"][number]["outcome"],
...
function finiteInteger(value: unknown, nonnegative = false): number | null {
  if (typeof value !== "number" || !Number.isFinite(value)) return null;
  const rounded = Math.round(value);
  return nonnegative ? Math.max(0, rounded) : rounded;
}

function nullableInteger(value: unknown, nonnegative = false): number | null {
  return value === null || value === undefined ? null : finiteInteger(value, nonnegative);
}
...
export function parseLambdaResultMessage(
  ...
  const laneTimingSummary = parseLambdaLaneTimingSummary(record.laneTimingSummary);
  ...
  if (parentDispatchSha256 && !/^[a-f0-9]{64}$/.test(parentDispatchSha256)) {
    throw new Error("Lambda result parent dispatch checksum is invalid.");
  }
  ...
  return {
    ...
    ...(laneTimingSummary ? { laneTimingSummary } : {}),
...
export async function recordLocalV2DagLambdaResult(
  ...
  if (existingEvent) {
    if (options.artifactVerification || parsedMessage.policyEvidence || parsedMessage.parentDispatchSha256) {
      await query(
        `update scan_events
            set metadata_json = metadata_json || jsonb_strip_nulls(jsonb_build_object(
              'artifactVerification', $2::jsonb,
              'policyEvidence', $3::jsonb,
              'parentDispatchSha256', $4::text
            ))
          where id = $1`,
        [
          existingEvent.id,
          ...
          parsedMessage.parentDispatchSha256 ?? null,
        ]
      );
    }
    if (artifactMirror) {
      ...
      [
          ...
          : "Local v2 DAG Lambda returned a completed artifact-only result.",
        {
          ...
          ...(parsedMessage.handlerTiming && Object.keys(parsedMessage.handlerTiming).length > 0
            ...
            : {}),
          ...(parsedMessage.laneTimingSummary
            ? { lambdaLaneTimingSummary: parsedMessage.laneTimingSummary }
            : {}),
          processor: PROCESSOR,
          policyEvidence: parsedMessage.policyEvidence ?? null,
          ...(parsedMessage.parentDispatchSha256
            ? { parentDispatchSha256: parsedMessage.parentDispatchSha256 }
            : {}),
          productionFindingIntegration: false,
