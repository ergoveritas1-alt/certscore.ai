...
function hasAiCue(documentText: string | null, title: string | null) {
  const haystack = `${title ?? ""}\n${documentText ?? ""}`;
  return /\bai\b|artificial intelligence|machine learning|automated system|algorithmic|chatbot|virtual assistant|generative ai|large language model|\bllm\b/i.test(
    haystack
  );
}

function inferAiFeatureClaims(documentText: string | null, title: string | null, parsedClaims: string[]) {
  const haystack = `${title ?? ""}\n${documentText ?? ""}`.toLowerCase();
  const claims = new Set(parsedClaims);

  if (/\bai[-\s]?powered\b|\bpowered by ai\b|artificial intelligence/.test(haystack)) {
    claims.add("ai_powered_feature");
  }
  if (/chatbot|virtual assistant|ai assistant|automated assistant/.test(haystack)) {
    claims.add("chatbot_or_virtual_assistant");
  }
  if (/automated recommendation|personalized recommendation|recommendation engine/.test(haystack)) {
    claims.add("automated_recommendation");
  }
  if (/generative ai|large language model|\bllm\b|ai-generated|synthetic content/.test(haystack)) {
    claims.add("generative_ai");
  }
  if (/machine learning|algorithmic model|automated scoring/.test(haystack)) {
    claims.add("machine_learning_or_scoring");
  }

  return [...claims];
}

function inferAiSensitiveContexts(documentText: string | null, title: string | null, parsedContexts: string[]) {
  if (!hasAiCue(documentText, title) && !/automated decision|profiling|automated processing/i.test(documentText ?? "")) {
    return parsedContexts;
  }

  const haystack = `${title ?? ""}\n${documentText ?? ""}`.toLowerCase();
  const contexts = new Set(parsedContexts);
  const contextPatterns: Array<[string, RegExp]> = [
    ["employment", /employment|hiring|recruitment|job applicant|workforce/],
    ["education", /education|student|admission|school|university/],
    ["credit", /credit|loan|lending|mortgage|financing/],
    ["insurance", /insurance|underwriting|premium/],
    ["healthcare", /healthcare|health care|medical|patient|clinical/],
    ["housing", /housing|tenant|rental|lease/],
    ["identity", /identity verification|id verification|identity proofing/],
    ["biometric", /biometric|face recognition|facial recognition|voice recognition/],
    ["minors", /children|minor|under 13|under 16|teen/]
  ];

  for (const [context, pattern] of contextPatterns) {
...
export function normalizeNanoDocumentExtraction(input: {
  ...
  const documentTitle = getString(input.row.title) ?? null;
  const aiFeatureClaims = inferAiFeatureClaims(
    input.documentText,
    documentTitle,
    getStringArray(input.parsed.ai_feature_claims ?? input.parsed.aiFeatureClaims)
  );
  const aiSensitiveContexts = inferAiSensitiveContexts(
    input.documentText,
    documentTitle,
    getStringArray(input.parsed.ai_sensitive_contexts ?? input.parsed.aiSensitiveContexts)
  );
  const aiHaystack = `${documentTitle ?? ""}\n${input.documentText ?? ""}`.toLowerCase();
  const inferredAiInteractionDisclosure =
    /(?:you are|users are|customer is).{0,80}(?:interacting|chatting|communicating).{0,80}(?:ai|artificial intelligence|chatbot|automated assistant)|(?:ai|automated) assistant|chatbot|virtual assistant/.test(aiHaystack);
  const inferredAiTransparencyNotice =
    /ai transparency|responsible ai|ai notice|how we use ai|use of artificial intelligence|artificial intelligence notice|automated decision-making|automated decision making/.test(aiHaystack);
  const inferredGeneratedContentLabel =
    /ai[-\s]?generated|generated by ai|synthetic content|artificially generated|deepfake/.test(aiHaystack);
  const inferredAutomatedDecisionDisclosure =
    /automated decision[-\s]?making|solely automated|algorithmic decision|automated processing|profiling|automated scoring/.test(aiHaystack);
  const inferredHumanReviewPath =
    /human review|request review|appeal|contest.{0,80}(?:decision|automated)|escalat(?:e|ion).{0,80}(?:human|agent|support)|contact us.{0,120}(?:automated|ai|decision)/.test(aiHaystack);

  const extractedFields: Record<string, unknown> = {
    ai_automated_decision_disclosure_present:
      getBoolean(input.parsed.ai_automated_decision_disclosure_present ?? input.parsed.aiAutomatedDecisionDisclosurePresent) ??
      inferredAutomatedDecisionDisclosure,
    ai_feature_claims: aiFeatureClaims,
    ai_generated_content_label_present:
      getBoolean(input.parsed.ai_generated_content_label_present ?? input.parsed.aiGeneratedContentLabelPresent) ??
      inferredGeneratedContentLabel,
    ai_human_review_path_present:
      getBoolean(input.parsed.ai_human_review_path_present ?? input.parsed.aiHumanReviewPathPresent) ??
      inferredHumanReviewPath,
    ai_interaction_disclosure_present:
      getBoolean(input.parsed.ai_interaction_disclosure_present ?? input.parsed.aiInteractionDisclosurePresent) ??
      inferredAiInteractionDisclosure,
    ai_sensitive_contexts: aiSensitiveContexts,
    ai_transparency_notice_present:
      getBoolean(input.parsed.ai_transparency_notice_present ?? input.parsed.aiTransparencyNoticePresent) ??
      inferredAiTransparencyNotice,
  ...
  const hasMeaningfulSemanticField =
    ...
    getStringArray(extractedFields.ai_feature_claims).length > 0 ||
    getStringArray(extractedFields.ai_sensitive_contexts).length > 0 ||
    extractedFields.ai_automated_decision_disclosure_present === true ||
    extractedFields.ai_generated_content_label_present === true ||
    extractedFields.ai_human_review_path_present === true ||
    extractedFields.ai_interaction_disclosure_present === true ||
    extractedFields.ai_transparency_notice_present === true ||
