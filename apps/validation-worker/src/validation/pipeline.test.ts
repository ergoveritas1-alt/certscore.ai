...
import {
  ...
  deriveCurrentScanReportValidationFindings,
...
test("current scan report derivation emits policy queue and GDPR section rows", () => {
  const findings = deriveCurrentScanReportValidationFindings(buildArtifacts());
...
test("current scan report validation derivation skips financial and accessibility review families", () => {
  const findings = deriveCurrentScanReportValidationFindings(
    buildArtifacts({
      ...
      pageEvidence: [
        {
          evidence_id: "financial-1",
          matched_text: "Earn guaranteed returns today.",
          page_type: "homepage",
          page_url: "https://example.com/"
        }
      ],
  ...
  assert.ok(findings.some((finding) => finding.ruleKey === "scan_report_review.policy_behavior_conflict_candidate"));
  assert.equal(findings.some((finding) => finding.ruleKey.startsWith("financial_review.")), false);
  assert.equal(findings.some((finding) => finding.ruleKey.startsWith("accessibility_review.")), false);
});
...
test("deriveUnifiedFindingsWithWorkflowEvents includes completion metadata on success", async () => {
  const events: Array<Record<string, unknown>> = [];
  ...
  const findings = await deriveUnifiedFindingsWithWorkflowEvents({
    ...
    completionMetadata: (items) => ({
      cookieDisclosureGapDiagnostic: {
        emitted: items.some((item) => item.ruleKey === "cookie_runtime.disclosure_gap"),
        reason: "emitted"
      }
    }),
    deriveFindings: () => [{ ruleKey: "cookie_runtime.disclosure_gap" }],
    scanId: "scan_123"
  });
...
test("undisclosed runtime cookie triggers cookie disclosure gap", () => {
  ...
  assert.deepEqual(
    ...
    {
      categoryDisclosurePresent: false,
      disclosedCookieRowCount: 1,
      emitted: true,
      ignoredRuntimeCookieCount: 0,
      policyExtractionStatus: "fetched",
      policySemanticConfidence: 0.92,
      policySourceUrl: "https://www.example.com/cookies",
      reason: "emitted",
      relevantRuntimeCookieCount: 1,
      runtimeCookieCount: 1,
      structurallyWeak: false,
      unmatchedCookieCount: 1,
      unmatchedCookieNames: ["_ga"],
