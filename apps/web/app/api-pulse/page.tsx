...
const mcpTools = certScoreMcpToolContracts.map(({ description, name }) => [name, description] as const);
    ...
    "certscore": {
      ...
      "env": {
        ...
        "CERTSCORE_BASE_URL": "https://certscore.ai"
            ...
            <p>
              Recommended workflow: call `certscore_scan_site` with a public URL. A new scan returns its stable <code>scanId</code> and may include a partial <code>preConsentPreview</code> when the runtime lane completes or reaches its six-second checkpoint. Captured totals are separate from bounded returned identities, and operational/security/consent vendors are separate from <code>trackingVendorCount</code>. Continue with `certscore_get_scan_status`
              while the scan is active, then use `certscore_list_findings` or `certscore_explain_finding` for review and ticketing. Fetch report,
