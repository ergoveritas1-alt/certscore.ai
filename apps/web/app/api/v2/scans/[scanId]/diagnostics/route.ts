import { apiV2ScanDiagnosticsSchema } from "@certscore/api-contracts";
import {
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return apiV2JsonResponse({
      body: apiV2ScanDiagnosticsSchema.parse(buildApiV2ScanDiagnostics(scanRecord)),
      requestId: id,
      route: "api-v2-scan-diagnostics",
      status: 200
    });
  } catch (error) {
    console.error("[api-v2-scan-diagnostics] request failed", { requestId: id, error });
    return apiV2JsonResponse({
