...
export default async function ProductAnalyticsPage({ searchParams }: Props) {
  ...
  const excludeInternal = resolved.audienceFilters === "1" ? resolved.excludeInternal === "1" : true;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="border-slate-200 bg-white"><CardHeader className="py-2"><CardTitle>Routes</CardTitle></CardHeader><CardContent className="grid gap-x-4 gap-y-0 pb-2 pt-0 sm:grid-cols-2 xl:grid-cols-3">{dashboard.routes.length ? dashboard.routes.map((row) => <div className="flex min-w-0 items-center justify-between gap-2 border-t border-slate-100 py-1 text-xs" key={row.route}><span className="font-medium text-slate-700">{row.route}</span><span className="truncate text-right text-[11px] text-slate-500">{count(row.events)} events · {count(row.sessions)} sessions</span></div>) : <p className="text-sm text-slate-400">No routes yet.</p>}</CardContent></Card>
        <Card className="border-slate-200 bg-white"><CardHeader className="py-2"><CardTitle>Feature activity</CardTitle></CardHeader><CardContent className="grid gap-x-3 gap-y-0 pb-2 pt-0 xl:grid-cols-3 2xl:grid-cols-5">{dashboard.features.length ? dashboard.features.map((row) => <div className="min-w-0 border-t border-slate-100 py-1 text-xs" key={`${row.eventName}:${row.feature}`} title={`${label(row.feature)} · ${label(row.eventName)} · ${count(row.events)} events · ${count(row.sessions)} sessions`}><div className="flex min-w-0 items-center justify-between gap-2"><span className="truncate font-medium text-slate-800">{label(row.feature)}</span><span className="shrink-0 text-[10px] text-slate-500">{count(row.events)} · {count(row.sessions)}</span></div><p className="truncate text-[9px] leading-3 text-slate-400">{label(row.eventName)}</p></div>) : <p className="text-sm text-slate-400">No feature activity yet.</p>}</CardContent></Card>
      </div>
      ...
      <Card className="min-w-0 overflow-hidden border-slate-200 bg-white">
        <CardHeader className="py-3"><div className="flex flex-wrap items-end justify-between gap-2"><div><CardTitle>Recent events</CardTitle><p className="mt-1 text-xs text-slate-500">Every row retains its source route. Scan evidence opens through the canonical scan record.</p></div><p className="text-sm text-slate-500">{count(eventPage.totalCount)} matching events</p></div></CardHeader>
        <CardContent className="min-w-0 space-y-3 pt-0">
          <AdminScansFilterForm basePath="/app/admin/analytics" clearHref="/app/admin/analytics" hasFilters={hasFilters} submitFirst>
            ...
            <input aria-label="Search events" className="h-10 min-w-[260px] flex-1 rounded-lg border border-slate-300 bg-white px-3 text-sm" name="q" placeholder="Route, endpoint, feature, event, source, actor, account, hostname" defaultValue={query} />
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200"><table className="min-w-full text-left text-xs"><thead className="border-b border-slate-200 bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500"><tr><th className="px-3 py-2">When</th><th className="px-3 py-2">Route</th><th className="px-3 py-2">Event</th><th className="px-3 py-2">Page / endpoint</th><th className="px-3 py-2">Feature</th><th className="px-3 py-2">User / session</th><th className="px-3 py-2">Context</th><th className="px-3 py-2">Scan</th></tr></thead><tbody>{eventPage.rows.length ? eventPage.rows.map((event) => <tr className="border-b border-slate-100 align-top last:border-0" key={event.event_id}><td className="whitespace-nowrap px-3 py-2 text-slate-500">{formatAdminDateTime(event.occurred_at)}</td><td className="px-3 py-2"><span className="rounded-full bg-slate-100 px-2 py-1 font-semibold text-slate-700">{event.event_route}</span></td><td className="px-3 py-2"><p className="font-medium text-slate-800">{label(event.event_name)}</p><p className="text-[10px] text-slate-400">{event.outcome}</p></td><td className="max-w-xs px-3 py-2"><code className="block truncate">{event.normalized_route}</code></td><td className="px-3 py-2 text-slate-600">{label(event.feature)}</td><td className="max-w-xs px-3 py-2"><p className="truncate">{event.email ?? event.actor_id?.slice(0, 12) ?? "Anonymous aggregate"}</p><p className="text-[10px] text-slate-400">{event.session_id?.slice(0, 12) ?? "No linkable session"}</p></td><td className="px-3 py-2 text-slate-500">{label(event.source)}<br />{event.device_class}{event.country_code ? ` · ${event.country_code}` : ""} · {event.consent_state}</td><td className="px-3 py-2">{event.scan_id ? <a className="text-sky-700 hover:underline" href={getAdminAuthenticatedScanHref(event.scan_id)}>{event.hostname ?? event.scan_id.slice(0, 8)}</a> : event.hostname ?? "—"}</td></tr>) : <tr><td className="px-3 py-8 text-center text-sm text-slate-400" colSpan={8}>No events match these filters.</td></tr>}</tbody></table></div>
        </CardContent>
      </Card>
