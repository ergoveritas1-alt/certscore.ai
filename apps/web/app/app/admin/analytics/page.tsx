...
type Props = { searchParams?: Promise<{ event?: string; includeBots?: string; includeInternal?: string; outcome?: string; page?: string; perPage?: string; period?: string; q?: string; route?: string }> };
...
export default async function ProductAnalyticsPage({ searchParams }: Props) {
  ...
  const [dashboard, eventPage] = await Promise.all([
    ...
    listProductAnalyticsEventsPage(period, includeInternal, includeBots, pageSize, (page - 1) * pageSize, { eventName, outcome, query, route })
  ]);
  ...
  const filterParams = { period, includeInternal: includeInternal ? "1" : null, includeBots: includeBots ? "1" : null, event: eventName, outcome, q: query, route };
    ...
    <div className="space-y-3">
      ...
      <Card className="overflow-hidden border-slate-200 bg-white">
        ...
        <CardContent className="p-0">
          ...
          <div className="border-t border-slate-100 p-3"><div className="mb-2 flex justify-between"><p className="text-xs font-semibold text-slate-700">Activity</p><p className="text-[10px] text-slate-400">{includeInternal ? "Internal/QA included" : "Internal/QA excluded"} · {includeBots ? "Bots included" : "Bots excluded"}</p></div><div aria-label="Event trend" className="flex h-16 items-end gap-1" role="img">{dashboard.trend.length ? dashboard.trend.map((point) => <div aria-hidden="true" className="min-w-0 flex-1 rounded-t bg-sky-500" key={point.bucket} style={{ height: `${Math.max(2, point.events / maxTrend * 64)}px` }} title={`${point.bucket}: ${point.events} events · ${point.sessions} sessions`} />) : <p className="text-sm text-slate-400">No activity in this period.</p>}</div></div>
        </CardContent>
      </Card>

      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="border-slate-200 bg-white"><CardHeader className="py-3"><CardTitle>Routes</CardTitle></CardHeader><CardContent className="space-y-1 pt-0">{dashboard.routes.length ? dashboard.routes.map((row) => <div className="flex items-center justify-between gap-3 border-t border-slate-100 py-2 text-sm" key={row.route}><span className="font-medium text-slate-700">{row.route}</span><span className="shrink-0 text-slate-500">{count(row.events)} events · {count(row.sessions)} sessions</span></div>) : <p className="text-sm text-slate-400">No routes yet.</p>}</CardContent></Card>
