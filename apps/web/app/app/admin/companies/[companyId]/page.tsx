...
async function CompanyDetailContent({ company, logoUrl, backHref }: { company: NonNullable<Awaited<ReturnType<typeof getCompanyDetail>>>; logoUrl: string | null; backHref: string }) {
  return (
    <div className="space-y-6">
      ...
      <Card className="border-slate-200 bg-white"><CardHeader><CardTitle>Add user</CardTitle><p className="text-sm text-slate-600">New users receive default access. The first user in a workspace receives advanced access automatically. We’ll email the user a secure link to choose their own password.</p></CardHeader><CardContent><form action={createCompanyUserFormAction} className="flex flex-col gap-3 sm:flex-row sm:items-end"><input name="companyId" type="hidden" value={company.id} /><label className="min-w-0 flex-1 text-sm font-medium text-slate-700">Email<input aria-label="Email address" className="mt-1 h-10 w-full rounded-lg border border-slate-300 px-3" name="email" required type="email" /></label><div className="shrink-0"><button className="app-raised-button app-raised-button-dark rounded-lg px-4 py-2 text-sm font-semibold text-white" type="submit">Create user and send invite</button></div></form></CardContent></Card>
