...
import { addExistingCompanyUserFormAction, removeCompanyLogoFormAction, removeCompanyUserFormAction, updateCompanyMembershipRoleFormAction, updateCompanyNameFormAction, uploadCompanyLogoFormAction } from "../../../../../server/company/actions";
...
async function CompanyDetailContent({ company, logoUrl, backHref }: { company: NonNullable<Awaited<ReturnType<typeof getCompanyDetail>>>; logoUrl: string | null; backHref: string }) {
  return (
    <div className="space-y-6">
      <Link className="text-sm font-semibold text-sky-700" href={backHref}>← Back to workspaces</Link>
      ...
      <Card className="border border-slate-200 bg-white"><CardHeader><CardTitle>Workspace details</CardTitle><p className="text-sm text-slate-600">Workspace names must be unique across CertScore.</p></CardHeader><CardContent><form action={updateCompanyNameFormAction} className="flex flex-col gap-3 sm:flex-row sm:items-end"><input name="companyId" type="hidden" value={company.id} /><label className="min-w-0 max-w-xl flex-1 text-sm font-medium text-slate-700">Workspace name<input className="mt-1 h-10 w-full rounded-lg border border-slate-300 px-3" defaultValue={company.name} name="name" required type="text" /></label><button className="app-raised-button app-raised-button-dark h-10 shrink-0 rounded-lg px-4 py-2 text-sm font-semibold text-white" type="submit">Save name</button></form></CardContent></Card>

      <Card className="border-slate-200 bg-white"><CardHeader><CardTitle>Workspace logo</CardTitle></CardHeader><CardContent className="space-y-3"><form action={uploadCompanyLogoFormAction} className="flex flex-wrap items-end gap-3"><input name="companyId" type="hidden" value={company.id} /><label className="text-sm font-medium text-slate-700">Upload logo<input accept="image/png,image/jpeg,image/webp,image/svg+xml" className="mt-1 block text-sm" name="logo" required type="file" /></label><button className="app-raised-button rounded-lg px-3 py-2 text-sm font-semibold" type="submit">Upload</button></form>{company.logoStorageKey ? <form action={removeCompanyLogoFormAction}><input name="companyId" type="hidden" value={company.id} /><button className="text-sm font-semibold text-rose-700" type="submit">Remove logo</button></form> : <p className="text-xs text-slate-500">PNG, JPEG, WebP, or SVG up to 2 MB.</p>}</CardContent></Card>

      <Card className="border-slate-200 bg-white"><CardHeader><CardTitle>Add existing user</CardTitle><p className="text-sm text-slate-600">Create the user from Admin → Users first. This assigns an unassigned user to this workspace.</p></CardHeader><CardContent><form action={addExistingCompanyUserFormAction} className="flex flex-col gap-3 sm:flex-row sm:items-end"><input name="companyId" type="hidden" value={company.id} /><label className="min-w-0 flex-1 text-sm font-medium text-slate-700">Email<input aria-label="Email address" className="mt-1 h-10 w-full rounded-lg border border-slate-300 px-3" name="email" required type="email" /></label><div className="shrink-0"><button className="app-raised-button app-raised-button-dark rounded-lg px-4 py-2 text-sm font-semibold text-white" type="submit">Add to workspace</button></div></form></CardContent></Card>
