...
import { getAdminScanSummary } from "../../../../../server/admin/get-admin-scan-detail";
import { AdminScanDeferredSections } from "./scan-diagnostic-sections";
...
export default async function AdminScanDetailPage({ params }: AdminScanDetailPageProps) {
  ...
  const record = await getAdminScanSummary(scanId);
    ...
    <div className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-slate-200 bg-white">
          ...
          <CardContent className="space-y-2 text-sm text-slate-600">
            <p>Tracker vendors: {record.collectedRowCounts.trackerVendors}</p>
            <p>Accessibility rules: {record.collectedRowCounts.accessibilityRules}</p>
            <p>Pages: {record.collectedRowCounts.pages}</p>
            <p>Changes: {record.collectedRowCounts.changes}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <AdminScanDeferredSections
        scanConfigJson={record.scan.scanConfigJson}
        scanFromLabel={record.scan.scanFromLabel}
        scanId={scanId}
      />
