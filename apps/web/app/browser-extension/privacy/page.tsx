    ...
    <main className="min-h-screen bg-white text-slate-900">
      ...
      <article className="mx-auto max-w-3xl space-y-8 px-6 py-16">
        <div className="space-y-3">
          ...
          <p className="text-base leading-7 text-slate-600">The CertScore.ai extension performs a user-initiated review of the website open in the selected Chrome tab and a bounded set of privacy, cookie, terms, and accessibility surfaces linked from that page.</p>
        </div>
        <section className="space-y-3"><h2 className="text-2xl font-semibold">Evidence collected</h2><p className="leading-7 text-slate-700">The extension may collect the target URL and hostname, bounded network request and response metadata, cookie names and attributes, visible consent-interface evidence, browser-observed fingerprinting signals, scan timing, transport/form/embed metadata, linked legal-surface URLs, bounded visible text from selected legal surfaces, and a screenshot of the visible target tab.</p></section>
        <section className="space-y-3"><h2 className="text-2xl font-semibold">How linked surfaces are reviewed</h2><p className="leading-7 text-slate-700">After the untouched pre-consent observation, the extension may open up to four discovered privacy, cookie, terms, or accessibility pages in background Chrome tabs, collect bounded evidence, and close those tabs. It does not click accept, reject, opt-out, settings, or other consent controls.</p></section>
        <section className="space-y-3"><h2 className="text-2xl font-semibold">Evidence not collected</h2><p className="leading-7 text-slate-700">The extension does not capture cookie values, passwords, form entries, payment information, or unrelated browsing activity.</p></section>
