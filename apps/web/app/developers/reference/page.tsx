...
- mcp`}</CodeBlock>
          <p className="max-w-3xl text-sm leading-7 text-slate-600">
            Scoped integrations use bearer API keys. Self-serve keys with{" "}
            <code className="rounded bg-white px-1">scan:read</code>, <code className="rounded bg-white px-1">scan:create</code>, and{" "}
            <code className="rounded bg-white px-1">mcp</code> are available for signed-in verified users through{" "}
            <code className="rounded bg-white px-1">POST /api/v2/keys/request</code>. For higher limits or custom access, email{" "}
            ...
            prefixed <code className="rounded bg-white px-1">cs_mcp_</code>, and are capped at 60 requests/minute and 500 scan/read
            requests/day.
