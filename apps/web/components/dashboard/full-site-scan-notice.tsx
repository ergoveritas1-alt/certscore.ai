...
export function FullSiteScanNotice({ scan, statusStale = false }: { scan: FullSiteScanNoticeData; statusStale?: boolean }) {
  ...
  return (
    <section aria-label="Full-site scan" className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {progress && scan.progress ? <div className="border-b border-slate-100 px-5 py-4">
        ...
        <div role="progressbar" aria-label="Full-site pages processed" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.percent} className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100">
        ...
        <p className="mt-2 text-xs text-slate-500">{statusStale ? "Status updates are delayed. Showing the last confirmed progress." : running ? (progress.seconds === null ? "Estimating time after the first page completes." : `About ${Math.max(1, Math.ceil(progress.seconds / 60))} min remaining · ~${Math.max(1, Math.ceil((progress.totalSeconds ?? 0) / 60))} min total${!scan.progress.discoveryComplete ? " at the page limit" : ""} · estimate based on observed page speed`) : "Progress reflects the recorded page outcomes."}</p>
      </div> : null}
