...
export function DomainScanForm({
  buttonLabel = "Start preview scan",
  helperText,
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const parsed = previewScanRequestSchema.safeParse({
      domain
    });
    ...
    try {
      const response = await fetch("/api/preview-scan", {
        body: JSON.stringify({
          domain
        }),
        headers: {
      ...
      if (!response.ok || !payload.previewUrl) {
        setErrorMessage(payload.error ?? "The preview scan could not be started. Please try again.");
      ...
      setErrorMessage("The preview scan could not be started. Please try again.");
      setIsSubmitting(false);
    }
  }
