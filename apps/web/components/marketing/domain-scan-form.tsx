...
import {
  ...
  readActiveScanSession,
  readPendingScanSession,
...
export function DomainScanForm({
  ...
  useEffect(() => {
    ...
    const activeScan = readActiveScanSession();
    if (!activeScan) return;

    let cancelled = false;
    const controller = new AbortController();
    fetch(`/api/scan-status/${encodeURIComponent(activeScan.scanId)}?includeFindings=0`, {
      cache: "no-store",
      ...
      .then(async (response) => {
        ...
        const payload = await response.json() as {
          scan?: { status?: string | null } | null;
        };
        const status = payload.scan?.status;
        const terminal = status === "completed" || status === "completed_limited" || status === "failed" || status === "canceled" || status === "cancelled" || status === "expired" || status === "rate_limited";
        const unsuccessfulTerminal = status === "failed" || status === "canceled" || status === "cancelled" || status === "expired" || status === "rate_limited";
        if (status && !unsuccessfulTerminal) {
          router.replace(activeScan.destination);
        } else if (terminal) {
          clearActiveScanSession(activeScan.scanId);
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    const pendingScan = readPendingScanSession();
    if (!pendingScan || pendingScan.mode !== mode) {
      return;
    }
    ...
    <form className={compact ? "space-y-2" : "space-y-4"} onSubmit={(event) => void handleSubmit(event)}>
      <div className="space-y-2">
        <div className="relative z-30">
          ...
          <Button
            ...
            className={
              variant === "homepage-hero"
                ...
                : compact
                ? scanButtonArmed
                  ? "absolute right-2 top-1/2 h-8 min-w-[4.5rem] -translate-y-1/2 rounded-full border border-slate-700 bg-[linear-gradient(180deg,#111827_0%,#020617_100%)] px-4 text-xs font-semibold text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_3px_0_0_rgba(148,163,184,0.45),0_9px_20px_-7px_rgba(14,165,233,0.5)] ring-1 ring-sky-300/50 transition-all duration-150 hover:-translate-y-[calc(50%+1px)] hover:scale-[1.03] hover:border-slate-600 hover:brightness-110 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_4px_0_0_rgba(148,163,184,0.4),0_12px_22px_-7px_rgba(14,165,233,0.6)] active:translate-y-[calc(-50%+1px)] active:scale-[0.99] active:shadow-[inset_0_2px_3px_rgba(0,0,0,0.35),0_1px_0_0_rgba(148,163,184,0.35)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-2"
                  : "absolute right-2 top-1/2 h-8 -translate-y-1/2 rounded-full border border-slate-600 bg-[linear-gradient(180deg,#334155_0%,#1e293b_100%)] px-4 text-xs font-semibold text-slate-200 opacity-80 shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_3px_0_0_rgba(148,163,184,0.3),0_8px_14px_-8px_rgba(15,23,42,0.65)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300 focus-visible:ring-offset-2"
          ...
          >
            {isSubmitting ? (
              <span className="whitespace-nowrap text-xs">Scanning…</span>
            ) : (
              variant === "homepage-hero" ? (
