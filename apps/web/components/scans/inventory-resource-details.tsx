...
const EvidenceContext = createContext<{ projection?: ApiRuntimeEvidenceGraphProjection; load: () => void; error?: string }>({ load() {} });
...
export function InventoryResourceProvider({ projection: initial, children }: { projection?: ApiRuntimeEvidenceGraphProjection; children: ReactNode }) {
  const [requested, setRequested] = useState(false);
  const [attempt, setAttempt] = useState(0);
  const [error, setError] = useState<string>();
  const [loaded, setLoaded] = useState<{ key: string; projection: ApiRuntimeEvidenceGraphProjection }>();
  const key = `${initial?.scanId}:${initial?.details?.sha256}:${initial?.sourceBundle?.sha256}`;
  useEffect(() => {
    if (!requested || !initial?.details || loaded?.key === key) return;
    ...
    return () => abort.abort();
  }, [requested, key, initial, loaded?.key, attempt]);
  return <EvidenceContext.Provider value={{ projection: loaded?.key === key ? loaded.projection : initial, error, load: () => { setRequested(true); if (error) setAttempt(value => value + 1); } }}>{children}</EvidenceContext.Provider>;
}
...
export function InventoryResourceRow({ children, identity, facts, inspect = false, relationships = true, existingDetails, evidence }: { children: ReactElement<{ children?: ReactNode; "data-resource-owner"?: string }>; identity: InventoryResourceIdentity; facts: Record<string, unknown>; inspect?: boolean; relationships?: boolean; existingDetails?: ReactNode; evidence?: Record<string, unknown> }) {
  ...
  const { projection, load, error } = useContext(EvidenceContext);
  ...
  const vendorCell = inspect && relationships && React.isValidElement<{ children?: ReactNode }>(cells[2]) ? cloneElement(cells[2], {}, <div className="flex min-w-0 items-center gap-1 whitespace-nowrap"><span className="inline-flex w-16 shrink-0"><RelationshipButton count={childEdges.length} open={treeOpen} unavailable={!graph && !projection?.details} deferred={Boolean(projection?.details)} onClick={() => { setTreeOpen(!treeOpen); load(); }} /></span><div className="min-w-0 truncate">{cells[2].props.children}</div></div>) : cells[2];
  ...
  return <>{row}<tr hidden={!open} data-resource-detail={id}><td colSpan={cells.length + (inspect ? 0 : 1)} className="bg-slate-50/60 px-5 py-4"><div id={id} className="w-[calc(100vw-7rem)] max-w-5xl">{open ? <ResourceDetails identity={identity} facts={facts} summary={existingDetails} evidence={evidence} /> : null}</div></td></tr>
    {inspect && treeOpen && (error || !childEdges.length) ? <tr data-relationship-status={id} data-resource-detail={id}><td colSpan={cells.length} className="bg-sky-50/40 px-5 py-3 text-xs text-slate-600"><p role="status">{error ?? (projection?.details ? "Loading retained relationship evidence…" : !graph ? "No relationship graph was retained for this scan’s pre-consent session. Resource identities alone cannot establish parent/child links." : !matches.length ? "No unambiguous graph resource match was retained for this row." : "No outgoing links were retained for this resource. This does not prove there were no children.")}</p>{error ? <button type="button" className={button} onClick={load}>Retry loading evidence</button> : null}</td></tr> : null}
...
function MainRelationshipRow({ graph, edge, path, depth }: { graph: ApiRuntimeEvidenceGraph; edge: ApiRuntimeEvidenceGraph["edges"][number]; path: string[]; depth: number }) {
  ...
  return <><tr data-main-relationship={node.id} className="border-b border-slate-100 bg-sky-50/40">
    ...
    <td className={cell}><div className="flex min-w-0 items-center gap-1" style={{ paddingLeft: indent }}><span className="inline-flex w-16 shrink-0 items-center border-l border-sky-200" ><span aria-hidden="true" className="text-sky-400">↳</span>{children.length && !cyclic ? <RelationshipButton count={children.length} open={expanded} onClick={() => setExpanded(!expanded)} /> : null}</span><span className="flex min-w-0 items-center gap-2"><VendorBrandIcon label={node.classification?.vendor ?? "Unknown"}/><span className="truncate">{node.classification?.vendor ?? "Unknown"}</span></span></div></td>
