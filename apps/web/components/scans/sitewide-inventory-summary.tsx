...
function Mix({ title, rows }: { title: string; rows: Array<{ label: string; count: number }> }) {
  ...
  const color = (label: string, index: number) => title === "Evidence mix"
    ...
    : label.toLowerCase() === "unknown" ? "#94a3b8" : colors[index % colors.length];
  ...
  return <section className="min-w-0 py-4 pr-4">
    ...
    <div className="mt-4 flex items-center gap-4">
      ...
      <ul className="max-h-36 min-w-0 flex-1 space-y-2 overflow-auto text-xs" aria-label={`${title}: ${total} resource identities`}>
        {rows.map((row, index) => <li key={row.label} className="flex items-center gap-2"><span aria-hidden="true" className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: color(row.label, index) }} /><span className="flex-1 capitalize">{row.label.replaceAll("_", " ")}</span><strong className="tabular-nums">{row.count}</strong></li>)}
...
export function SitewideInventorySummary({ mix }: { mix: FullSiteReportResponse["inventoryMix"] }) {
  return <section aria-label="Sitewide resource inventory" className="my-4 border-y border-zinc-200 bg-white">
    ...
    <div className="grid divide-y divide-zinc-200 lg:grid-cols-3 lg:divide-y-0"><Mix title="Evidence mix" rows={mix.evidence} /><Mix title="Purpose mix" rows={mix.purpose} /><Mix title="Site relationship" rows={mix.relationship} /></div>
  </section>;
}
