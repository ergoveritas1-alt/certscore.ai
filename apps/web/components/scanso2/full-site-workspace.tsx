  ...
  return (
    ...
    >
      <header className="border-b border-zinc-200 pb-3">
        ...
        <div className="mt-3 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:items-start lg:gap-x-6 lg:[&>div:first-child]:contents lg:[&>div:first-child>header]:contents lg:[&>div:first-child>header>div:last-child]:col-span-full">
          <div>{completed && tab !== "resources" ? identity : identityWithoutSharing ?? identity}</div>
          {completed ? <div className="mt-3 flex justify-end lg:col-start-2 lg:row-start-1 lg:mt-0 lg:w-full lg:max-w-xl lg:justify-self-end">{scanNext}</div> : null}
        </div>
      ...
      >
        {(["resources", "pages", "homepage"] as const).map((value) => (
          ...
          >
            {value === "homepage"
              ? "Homepage report"
              : value === "pages" ? "Page coverage" : "Full site report"}
          </button>
        ))}
      </nav>
...
<div className="mb-3 flex flex-wrap items-center justify-between gap-3">            <h2 className="text-xl font-semibold">{tab === "pages" ? "Page observations and coverage" : "Resources and Services Details"}</h2>{tab !== "pages" ? <div className="flex shrink-0 gap-2" role="group" aria-label="Inventory view">{(["services", "resources"] as const).map(view => <button key={view} type="button" aria-pressed={inventoryView === view} className={`${button} capitalize ${inventoryView === view ? "!bg-slate-900 !text-white" : ""}`} onClick={() => setInventoryView(view)}>{view}</button>)}</div> : null}</div>
          {data && tab === "resources" ? <SitewideInventorySummary mix={data.inventoryMix} updating={valuesUpdating} /> : null}
            ...
            {<p className="mt-2 text-xs text-zinc-500">{!data ? "Loading inventory…" : isFetching ? "Updating inventory…" : `${tab === "pages" ? data.pages.total : inventoryView === "services" ? data.services.length : data.resources.total} rows · Up to 6 visible. Scroll for more.`}</p>}
          </section>
