...
export function useTableRowLimit(limit: number) {
  ...
  useLayoutEffect(() => {
    ...
    if (!root) return;
    const measure = () => {
      const table = [...root.querySelectorAll("table")].find(table => table.offsetHeight > 0);
      if (!table) return;
      ...
      setMaxHeight(header + rows.slice(0, limit).reduce((sum, row) => sum + row.getBoundingClientRect().height, 0) + 2);
    };
