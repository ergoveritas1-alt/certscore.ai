...
const METHODOLOGY: Record<string, string> = {
  ...
  fingerprinting_related_signals_observed:
    "CertScore inspects retained runtime evidence for browser, device, canvas, audio, storage, font, plugin, screen, locale, timing, and other high-entropy environment signals where available. The finding is surfaced when retained evidence indicates one or more fingerprinting-related or device-signal observations, but the retained cluster is not strong enough for probable fingerprinting. CertScore treats fingerprinting-related results as review signals. The scanner does not determine personal identity, identity resolution, persistent fingerprint creation, user singling-out, legal status, consent validity, or compliance status. Reviewers should consider purpose, entropy, signal count, vendor role, consent state, security or fraud-prevention context, downstream use, and whether retained evidence distinguishes functional device checks from tracking or profiling uses.",
  session_recording_services_detected:
...
const LIMITATIONS: Record<string, string[]> = {
  ...
  fingerprinting_related_signals_observed: [
    ...
    "CertScore redacts or avoids retaining raw attribute values, full query strings, identifiers, payloads, screenshots, and sensitive values while preserving stable anchors needed for review."
  ],
  probable_fingerprinting: [
