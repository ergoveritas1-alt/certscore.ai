...
test("Pulse docs page source includes integration-critical guidance", () => {
  ...
  assert.match(source, /remote Streamable HTTP endpoint/);
  assert.match(source, /local stdio command/);
  assert.match(source, /pnpm mcp:certscore/);
  assert.match(source, /pnpm mcp:certscore:smoke/);
  assert.match(source, /Read-only MCP access uses `scan:read` and `mcp`/);
  assert.match(source, /Starting a scan requires `scan:create`/);
  assert.match(source, /Example MCP client config/);
  assert.match(source, /Recommended workflow/);
  assert.match(source, /without changing evidence, concern, finding, or/);
  assert.match(source, /projection logic/);
  ...
  assert.match(source, /Account browsing and drift comparison tools/);
  assert.match(source, /not part of the MCP scope/);
...
test("Pulse agent fallback page documents the fetch failure diagnostic contract", () => {
  ...
  assert.match(source, /scanFrom/);
  assert.match(source, /geo/);
  assert.match(source, /eu_ie<\/code>/);
...
test("Pulse plain text agent guide is retrievable and covers fetch failures", () => {
  ...
  assert.match(source, /https:\/\/certscore\.ai\/developers\/mcp/);
  assert.match(source, /https:\/\/mcp\.certscore\.ai\/mcp/);
  assert.match(source, /https:\/\/mcp\.certscore\.ai\/\.well-known\/oauth-protected-resource/);
  ...
  assert.match(source, /Read-only \+ MCP keys are self-serve/);
  assert.match(source, /scan:create is grant-gated/);
...
test("Developer API docs are discoverable by crawlers and agent manifests", async () => {
  ...
  assert.equal(aiDiscovery.developerDocs.agentQuickstart, "https://certscore.ai/developers#agent-quickstart");
  assert.equal(aiDiscovery.developerDocs.completeCurlWorkflow, "https://certscore.ai/developers/quickstart#complete-curl-workflow");
  assert.equal(aiDiscovery.developerDocs.genericAgentInstructions, "https://certscore.ai/developers/examples#generic-agent-instructions");
  ...
  assert.equal(aiDiscovery.mcp.currentVersion, "0.2.4");
  assert.equal(
  ...
  assert.deepEqual(aiDiscovery.authentication.currentScopes, ["scan:read", "mcp"]);
  assert.deepEqual(aiDiscovery.authentication.grantGatedScopes, ["scan:create"]);
  ...
  assert.deepEqual(aiDiscovery.authentication.recommendedScopes.mcpReadOnly, ["scan:read", "mcp"]);
  assert.deepEqual(aiDiscovery.authentication.recommendedScopes.mcpScanCreation, ["scan:read", "scan:create", "mcp"]);
  ...
  assert.match(llmsFull, /Agent quick path/);
  ...
  assert.match(combinedSources, /id="agent-quickstart"/);
  assert.match(combinedSources, /id="complete-curl-workflow"/);
  assert.match(combinedSources, /id="generic-agent-instructions"/);
  assert.match(combinedSources, /id="mcp-agent-workflow"/);
  assert.match(combinedSources, /id="pre-consent-cookies-trackers"/);
