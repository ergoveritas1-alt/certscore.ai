...
test("Pulse plain text agent guide is retrievable and covers fetch failures", () => {
  ...
  assert.match(source, /certscore_explain_finding for evidence summaries, caveats, and next steps/);
...
test("Light MCP onboarding is no-auth, copyable, and agent-complete", () => {
  ...
  const publicScanPage = readFileSync("apps/web/app/(marketing)/scano/[scanId]/page.tsx", "utf8");
  ...
  assert.match(lightPage, /retry certscore_scan_site if a retryable error has no scanId/);
  assert.match(lightPage, /certscore_get_scan_status with scanId if still running/);
  assert.match(lightPage, /certscore_get_scan_bundle after terminal status/);
  ...
  assert.match(lightPage, /retry .*certscore_scan_site.* only when the error says .*retryable: true/);
...
test("Developer API docs are discoverable by crawlers and agent manifests", async () => {
  ...
  assert.equal(aiDiscovery.api.anonymousAgentScan.dailyNewScanLimit, 20);
  ...
  assert.equal(aiDiscovery.mcp.currentVersion, "0.2.20");
  assert.equal(aiDiscovery.mcp.hosted.endpoint, "https://mcp.certscore.ai/mcp");
  assert.equal(aiDiscovery.mcp.hosted.currentVersion, "0.2.20");
  ...
  assert.equal(aiDiscovery.mcp.anonymous.dailyNewScanLimit, 20);
  ...
  assert.equal(aiDiscovery.mcp.light.version, "0.2.20");
