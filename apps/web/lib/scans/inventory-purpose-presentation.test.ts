...
test("unknown purpose uses only an unambiguous retained site relationship", () => {
  assert.equal(inventoryPurposeLabel("unknown", ["first_party"]), "Unknown – 1st party");
  assert.equal(inventoryPurposeLabel("unknown", ["third_party"]), "Unknown – 3rd party");
  ...
  assert.equal(inventoryPurposeTitle("Unknown – 1st party"), "Purpose unknown; first-party resource");
});

test("presentation preserves known purposes and retained classifications", () => {
  const purposes = ["analytics", "unknown"];
  assert.deepEqual(inventoryPurposeGroups(purposes, ["first_party"]), ["analytics", "Unknown – 1st party"]);
