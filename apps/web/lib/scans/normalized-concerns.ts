...
function isReplayConcern(concern: Pick<NormalizedConcern, "canonicalConcernKey" | "suggestedUnifiedFindingId" | "originKey" | "title">) {
  const haystack = [
...
function buildConcernFromSharedInput(input: {
  ...
  const eligibility = deriveConcernEligibility({
    concern: {
      canonicalConcernKey,
      originKey: input.originKey,
      originType: input.originType,
