...
const COOKIE_NAME_SIGNATURES: Array<{ signatureId: string; vendor: string; category?: string; pattern: RegExp }> = [
...
export function findRuntimeCookieNameVendor(cookieName: string | null | undefined) {
  ...
  return signature ? {
    ...
    attributionEvidence: { signatureId: signature.signatureId, matchedOn: "cookie_name" as const, matchedValue: trimmed }
  } : null;
}
