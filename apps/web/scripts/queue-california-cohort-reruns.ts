...
import type { QueuedFullScanCaliforniaPrivacyConfig } from "../server/scans/full-scan-config";

const DEFAULT_CALIFORNIA_COHORT_DOMAINS = [
...
const DEFAULT_SOURCE = "california-cohort-validation";
...
function getDatabaseUrlForSafetyCheck() {
  return process.env.DATABASE_URL ?? process.env.POSTGRES_URL ?? null;
}

function getDatabaseHostForSafetyCheck() {
  const databaseUrl = getDatabaseUrlForSafetyCheck();
  if (databaseUrl) {
    try {
...
function buildCaliforniaPrivacyConfig(): QueuedFullScanCaliforniaPrivacyConfig {
  return {
    exercisePrivacyChoicePath: !hasFlag("--skip-privacy-choice-exercise"),
    forceGpcVerification: !hasFlag("--skip-gpc-verification")
  };
}
...
async function queueCandidate(input: {
  californiaPrivacy: QueuedFullScanCaliforniaPrivacyConfig;
  ...
  try {
    ...
    await insertScanEvent({
      ...
      message: "Prior scan acceleration skipped for operator California cohort rerun.",
      metadataJson: {
        californiaPrivacy: scanConfig.californiaPrivacy ?? null,
        found: false,
        reason: "operator_california_cohort_rerun",
        source: input.source
      },
      scanId: scan.id
    });
    ...
    await insertScanEvent({
      ...
      message: "California cohort validation scan queued and awaiting scanner pickup.",
      metadataJson: {
        californiaPrivacy: scanConfig.californiaPrivacy ?? null,
        pagesRequested: input.pagesRequested,
        profile: input.profile,
...
async function main() {
  ...
  const scanFrom = normalizeScanFrom(getArgValue("--scan-from") ?? getArgValue("--geo"));
  ...
  const californiaPrivacy = buildCaliforniaPrivacyConfig();
  const domains = getListArg("--domains");
  const { candidates, invalid } = normalizeDomains(domains.length > 0 ? domains : DEFAULT_CALIFORNIA_COHORT_DOMAINS);
