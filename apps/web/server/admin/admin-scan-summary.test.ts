...
test("admin scan summaries consume the canonical report projection", async () => {
  ...
  assert.match(source, /score: resolveScanReportScore\(/);
  assert.match(source, /noGo\.isNoGo \? null : recordNumber\(reportSummary, "score"\)/);
...
test("Admin Scans separates requester identity from outbound scanner egress", async () => {
  ...
  assert.match(pageSource, /\{ label: "Requester IP" \}, \{ label: "Requested" \}, \{ label: "Page" \}, \{ label: "Created via" \}/);
