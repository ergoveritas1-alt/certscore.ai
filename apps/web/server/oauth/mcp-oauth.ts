...
function normalizeRedirectUris(value: unknown) {
  ...
  return raw
    ...
    .filter(Boolean)
    .filter(isAllowedMcpOAuthRedirectUri)
    .slice(0, 20);
}
...
export async function exchangeAuthorizationCode(input: {
  ...
  const candidate = await queryOne<AuthorizationCodeRow>(
    `select client_id, redirect_uri, code_challenge, code_challenge_method, scope,
            organization_id, owner_user_id, expires_at, consumed_at
  ...
  if (
    ...
    candidate.redirect_uri !== input.redirectUri ||
    !verifyPkceS256(input.codeVerifier, candidate.code_challenge)
  ) {
    return null;
  }
