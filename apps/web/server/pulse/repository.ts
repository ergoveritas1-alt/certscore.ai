...
export function pulseScanThrottleIdentity(input: { normalizedDomain: string; normalizedUrl: string }) {
  try {
    const url = new URL(input.normalizedUrl);
    const path = url.pathname.replace(/\/{2,}/g, "/");
    const normalizedPath = path.length > 1 ? path.replace(/\/$/, "") : "/";
    return normalizedPath === "/" ? input.normalizedDomain : `${input.normalizedDomain}${normalizedPath}`;
  } catch {
    return input.normalizedDomain;
  }
}

export async function claimPulseDomainScanCreation(input: { normalizedDomain: string; normalizedUrl: string; pulseRequestId: string }) {
  await ensurePulseTables();
