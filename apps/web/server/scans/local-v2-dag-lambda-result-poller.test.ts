...
test("artifact mirror downloads durable Lambda artifacts into the local v2 DAG scan directory", async () => {
  ...
  const fullPageScreenshotBody = Buffer.from("not-a-real-jpeg-for-mirror-test");
  const objects = new Map([
    ["v2/scan-local-1/LocalV2DagLambdaManifest.json", Buffer.from(JSON.stringify({
      auxiliaryArtifacts: [
        ...
        },
        {
          ...
          uri: "s3://certscore-dev-artifacts/v2/scan-local-1/auxiliary/screenshot-pre-consent-full-page.jpg"
        }
      ],
      manifest: true
    }))],
    ...
    ["v2/scan-local-1/auxiliary/screenshot-pre-consent-full-page.jpg", fullPageScreenshotBody]
  ]);
  ...
  assert.ok(mirror);
  assert.equal(mirror.mirroredArtifacts.length, 7);
  assert.equal(
  ...
  assert.equal(
    await readFile(path.join(workspaceRoot, "artifacts/local-v2-dag-scans/scan-local-1/screenshot-pre-consent-full-page.jpg"), "utf8"),
    "not-a-real-jpeg-for-mirror-test"
  );
...
test("local web poller records Lambda result before marking scan completed", async () => {
  ...
  assert.match(source, /blocked_flag = excluded\.blocked_flag/);
  assert.match(source, /captcha_flag = excluded\.captcha_flag/);
  assert.match(source, /insert into scan_snapshots/);
  ...
  assert.match(source, /scannerBuildProvenanceStatus/);
  assert.match(source, /scannerExecutionMode/);
  assert.match(source, /wc01Projection/);
  assert.match(source, /LOCAL_V2_DAG_WC01_PROJECTION_VERSION/);
