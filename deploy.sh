...
deploy_args=(
  ...
  --memory 2Gi
  --cpu 2
  --set-env-vars "NODE_ENV=production,NEXT_PUBLIC_SUPABASE_URL=${NEXT_PUBLIC_SUPABASE_URL},NEXT_PUBLIC_SUPABASE_ANON_KEY=${NEXT_PUBLIC_SUPABASE_ANON_KEY},SUPABASE_STORAGE_BUCKET=${SUPABASE_STORAGE_BUCKET},SUPABASE_STORAGE_BUCKET_SCREENSHOTS=${SUPABASE_STORAGE_BUCKET_SCREENSHOTS:-scan-screenshots},SUPABASE_STORAGE_BUCKET_ARTIFACTS=${SUPABASE_STORAGE_BUCKET_ARTIFACTS:-scan-artifacts},WORKER_CONCURRENCY=${WORKER_CONCURRENCY:-2},PLAYWRIGHT_BROWSERS_PATH=${PLAYWRIGHT_BROWSERS_PATH:-/ms-playwright},LLM_ENRICHMENT_ENABLED=${LLM_ENRICHMENT_ENABLED:-0},LLM_ENRICHMENT_TIMEOUT_MS=${LLM_ENRICHMENT_TIMEOUT_MS:-15000},LLM_ENRICHMENT_MAX_ATTEMPTS=${LLM_ENRICHMENT_MAX_ATTEMPTS:-3},LLM_ENRICHMENT_MAX_CHUNKS=${LLM_ENRICHMENT_MAX_CHUNKS:-5},LLM_ENRICHMENT_FORCE_LAST_CHUNK=${LLM_ENRICHMENT_FORCE_LAST_CHUNK:-1}"
