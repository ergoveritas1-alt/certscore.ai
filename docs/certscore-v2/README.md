...
- can run a bounded consent-flow runtime scan as a separate module
...
`consentFlowRuntimeScanner` launches bounded fresh-context scenarios for baseline pre-consent, reject-all, and accept-all flows. It observes likely consent controls, records screenshots/DOM artifacts before and after attempted actions, tags runtime events by scenario and consent state, and emits cross-phase comparisons for persisted/suppressed/accept-only vendors, cookies, collection endpoints, and journey phase deltas. Deterministic text/role/ARIA control detection runs first, and Nano UI assist is mandatory at the scan entrypoint for consent UI classification support. Tests use mock providers and Nano cannot force weak-evidence or unknown-control clicks. Consent comparisons now carry action-confidence limitations for every phase involved in the comparison; direct accept-vs-reject deltas require both accept and reject phases to be confidently executed. Reject-all flows prefer a direct reject control and may fall back to a bounded manage/preferences traversal when a high-confidence first-layer preferences control opens a second layer with clear reject/save controls. Manage/preferences-only observation is not treated as reject success. Screenshot capture is best-effort and falls back to a tiny placeholder artifact rather than failing an otherwise useful runtime module.
