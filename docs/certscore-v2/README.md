...
Pre-consent network capture also retains CDP `remoteIPAddress` as a network destination. When local MaxMind GeoLite2 City and ASN databases are available, scan-core adds country, city, ASN, and provider without making a remote lookup. Configure `CERTSCORE_GEOLITE2_CITY_DB_PATH` and `CERTSCORE_GEOLITE2_ASN_DB_PATH`, or mount the Lambda files at `/opt/geolite2/GeoLite2-City.mmdb` and `/opt/geolite2/GeoLite2-ASN.mmdb`. Reports label this evidence as **server location (may be CDN edge)**; it is not treated as a data-storage location. If the databases are unavailable, the CDP IP is still retained and the geographic fields remain unset.
