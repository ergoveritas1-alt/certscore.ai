...
- The Official MCP Registry release target is `0.2.20`. Its API also retains older immutable versions, including the stale `0.2.12` 20-scan description; downstream indexes must select the registry record marked `isLatest: true`.
...
1. Keep one canonical identity everywhere: `CertScore.ai MCP Light`, `ai.certscore/mcp-light`, hosted MCP `0.2.20`, publisher `CertScore.ai, LLC`.
...
2. Submit the prepared Claude package using publisher `CertScore.ai, LLC` and version `0.2.20`.
