...
- Decision schema v2 makes Luna's monitoring baselines a mandatory cutover input. Until Luna approves every bounded sample-count, drift, contradiction, withholding, region-variance, and source-variance limit with an evidence artifact, the production monitor withholds alert judgment. Invalid or incomplete approved baselines fail closed; undersampling or an exceeded approved limit pauses rollout.
