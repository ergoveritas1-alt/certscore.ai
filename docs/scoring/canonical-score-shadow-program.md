...
- Every comparison artifact records model version, source, score kind, input fingerprint, legacy score metadata, and score delta. Schema v6 also records separate SHA-256 fingerprints and counts for canonical coverage-row states and score-eligible finding tuples. These bounded diagnostics contain no target identity, raw evidence, or free-form text and explain whether a cross-source mismatch came from collection coverage or finding projection.
- Coordinated Lambda/browser collection must follow the explicit, immutable pair contract in `docs/scoring/deliberate-source-pair-contract.md`. Pair membership is diagnostic only and never substitutes for whole-input fingerprint equality.
...
- Decision schema v3 makes Luna's monitoring baselines a mandatory cutover input. Until Luna approves every bounded sample-count, drift, contradiction, withholding, region-variance, same-region source-variance, and identical-input source-variance limit with an evidence artifact, the production monitor withholds alert judgment. Invalid or incomplete approved baselines fail closed; undersampling or an exceeded approved limit pauses rollout.
- Source monitoring has two deliberately different lanes. `crossSource` compares different scanner source families only when the requested URL and known region match. `equivalentInputCrossSource` compares different source families only when the requested URL and canonical score-input fingerprint match exactly; it may retain unknown browser geography because it proves scorer equivalence conditioned on identical bounded input rather than geographic collection equivalence. Different fingerprints can never enter that lane.
