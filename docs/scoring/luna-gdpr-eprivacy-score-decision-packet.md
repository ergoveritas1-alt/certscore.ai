...
5. **Monitoring baselines:** approve minimum seven-day sample, comparable-scan, cross-region-group, same-region cross-source-group, and identical-input cross-source-group counts plus maximum score-drift p95, contradiction rate, withholding rate, cross-region range, same-region source range, and identical-input source range. No alert judgment is made while these values remain pending.
...
EU-DE, EU-IE, and browser-extension rows. Those fingerprints are different, and a
bounded 90-day production query found zero cross-source groups sharing both target
and exact input fingerprint, so no identical-input group is credited yet. This is
the expected fail-closed result and leaves the live Lambda/browser-extension sample
requirement open.
...
- A separate identical-input source lane may include unknown browser geography only when hostname key, exact requested-URL key, and canonical score-input fingerprint all match. It proves deterministic scorer equivalence for the same bounded input; it does not prove equivalent regional collection behavior.
