...
| Versioned bounded shadow comparisons | Schema v4 artifacts are bounded and versioned, carry projection fingerprints, both coverage meanings, deltas, accepted migration differences, and unresolved contradictions. | Proved |
...
| Production monitoring | Completion lifecycle persists immutable bounded shadow comparison metrics for drift, contradictions, withholding, and hashed cross-region groups. The admin monitor summarizes a seven-day window. Owned canary `52d2ca2e-5913-428a-8f5d-f162c68eca0d` produced a candidate-v3 shadow assessment and comparison row in production: legacy 94, candidate 100, delta +6, zero unresolved contradictions. Luna-approved alert baselines remain pending. | Deployed and first row verified; alert thresholds pending |
...
4. Pass `pnpm score:luna-cutover-gate`, enable the exact approved version, and verify all production consumers before claiming completion.
