...
| Controlled rollout flag and rollback | A single disabled-by-default selector now spans report, Pulse exports, dashboard/history, and admin. It requires exact Luna approval and exact kind/version, preserves candidate withholding, and fails closed to legacy. The rollback runbook restores legacy selection without rewriting history. | Implemented; deployment and approved enablement pending |
| Web, Pulse, dashboard, exports, and admin agreement | All consumers use the same configured persisted selection and preserve kind/version/coverage metadata. Candidate equivalence still requires production verification after final approval. | Implemented; production cutover verification pending |
| Production monitoring | Completion lifecycle persists immutable bounded shadow comparison metrics for drift, contradictions, withholding, and hashed cross-region groups. The admin monitor summarizes a seven-day window. Luna-approved alert baselines and deployed-row verification remain pending. | Implemented baseline; thresholds and production verification pending |
