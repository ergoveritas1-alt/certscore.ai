...
function hasScanCoreRowSpecificArticle13Terms(
  ...
  switch (disclosureType) {
    ...
    case "supervisory_authority":
      return /\b(?:(?:lodge|file|submit|make)\s+a\s+complaint.{0,160}(?:supervisory|data protection|regulator|authority)|complaints?.{0,200}(?:data protection authorit(?:y|ies)|supervisory authorit(?:y|ies)|regulator)|complain to (?:(?:a|your|the|our|local)\s+)?(?:(?:data protection|supervisory)\s+)?(?:regulator|authority)|(?:supervisory authority|data protection authority|local data protection authorit(?:y|ies)).{0,160}complaint|compliance (?:and|&) cooperation with regulators.{0,320}(?:complaints?|regulatory authorities|local data protection authorities|resolve)|formal written complaints?.{0,180}(?:regulatory authorities|local data protection authorities|regulators?)|unresolved complaints?.{0,180}(?:regulatory authorities|local data protection authorities|regulators?)|regulators?.{0,120}(?:complaints?|authorities|resolve))\b/i.test(text);
