...
export const scanLaneRunSchema = z.object({
  ...
  firstEffectiveUrl: z.string().max(500).nullable(),
  navigationCount: z.number().int().nonnegative(),
  navigationAttempts: z.array(z.object({
...
export const cookieEventSchema = runtimeEvidenceEventSchema.extend({
  ...
  cookieEssentiality: z.enum(["essential", "non_essential", "unknown"]).default("unknown"),
  cookieEssentialityConfidence: z.number().min(0).max(1).optional(),
  cookieEssentialityReasonCodes: z.array(z.string().max(120)).max(12).default([]),
...
export const policySurfaceObservationSchema = z.object({
  ...
  effectiveDate: z.string().max(80).optional(),
  directlyLinkedFromScannedPage: z.boolean().optional(),
  translationApplied: z.boolean().optional(),
  translationTargetLanguage: z.string().max(40).optional(),
  textExcerpt: z.string().optional(),
...
export function deriveConsentSurfaceInspectionOutcome(input: {
  ...
  const supersededConsentRecoveryLimitations = new Set([
    // A verified recovery only completes the consent-surface lane. General
    // runtime coverage remains limited and is retained unchanged in the
    // bundle; these keys are ignored only for this derived inspection.
    "pre_consent_runtime_partial",
