...
test("OAuth and Light preserve form inventory and snapshot states, forward cursors, and guide continuation", async () => {
  const scanId = '9ba99a8c-b1ad-44c1-985f-92cef760ab40';
  const cursor = `v1.${'a'.repeat(64)}.1`;
  const page = { type: 'certscore_report_evidence_page', version: 1, scanId, snapshot: 'a'.repeat(64), reportUrl: `https://certscore.ai/scan/${scanId}`, entries: [{ path: '/collectionTableRows', value: ['available', 'unavailable', 'withheld'].map((status, index) => ({ id: `form_${index}`, form: { formRef: `form_${index}`, fields: [{ fieldRef: `field_${index}`, label: 'Email', required: true }], fieldsTruncated: true, candidateFieldCount: 21, retainedFieldCount: 20 }, snapshot: status === 'available' ? { status, url: `/api/scans/${scanId}/form-snapshot?formRef=form_${index}` } : { status, reason: status === 'withheld' ? 'review_withheld' : 'review_failed' } })) }], pagination: { offset: 0, returned: 1, total: 2, complete: false, nextCursor: cursor }, coverage: { scope: 'public_report_projection', exportTruncated: false, observationCompleteness: 'see_report_coverage', exclusions: [] }, reconstruction: 'JSON Pointer entries', download: { url: `https://certscore.ai/api/v2/scans/${scanId}/report-evidence?format=download`, mediaType: 'application/json', bytes: 790000, authentication: 'same_access_rules_as_mcp', instructions: 'Use MCP pagination if authenticated downloads are unavailable.' } };
