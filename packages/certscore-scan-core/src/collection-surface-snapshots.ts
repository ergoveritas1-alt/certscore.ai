  ...
  for (const form of inventory.forms) {
    ...
    try {
      ...
      target = await page.evaluateHandle(({ fields, structure, url }) => {
        ...
        if (controls.some((el, i) => !el || el.tagName.toLowerCase() !== fields[i]!.elementType || (el.getAttribute("type") || el.tagName.toLowerCase()).toLowerCase() !== fields[i]!.inputType)) return null;
        ...
        if (controls.some((el, i) => labelFor(el!) !== fields[i]!.label || (el as HTMLInputElement).required !== fields[i]!.required)) return null;
      ...
      const original = await element.screenshot({ type: "jpeg", quality: 45, scale: "css", timeout: Math.min(1000, remaining), mask: [page.locator("input, textarea, select, [contenteditable]")], maskColor: "#94a3b8", caret: "hide" });
