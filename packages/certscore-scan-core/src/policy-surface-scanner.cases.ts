...
const policyTestShard = parsePolicyTestShard(process.env.CERTSCORE_POLICY_TEST_SHARD);
let policyTestIndex = 0;
const test = new Proxy(nodeTest, {
  apply(target, thisArgument, argumentsList) {
    const currentTestIndex = policyTestIndex;
    policyTestIndex += 1;
    if (policyTestShard && currentTestIndex % policyTestShard.total !== policyTestShard.index) {
      return undefined;
    }
...
function parsePolicyTestShard(value: string | undefined): { index: number; total: number } | undefined {
  if (!value) return undefined;
  const match = /^(\d+)\/(\d+)$/.exec(value);
  if (!match) throw new Error(`Invalid CERTSCORE_POLICY_TEST_SHARD value: ${value}`);
  const index = Number(match[1]);
  const total = Number(match[2]);
  if (!Number.isInteger(index) || !Number.isInteger(total) || total < 1 || index < 0 || index >= total) {
    throw new Error(`Invalid CERTSCORE_POLICY_TEST_SHARD value: ${value}`);
  }
  return { index, total };
}
  ...
  const candidates = gdprTransparencyTopicCandidatesFromRetainedPolicySections([
    {
      ...
      textExcerpt:
        "Our hosting and content-delivery providers may process ordinary request information such as IP address, requested URL, timestamp, browser information, and security diagnostics to deliver and protect the site."
    },
    {
      heading: "Analytics and session diagnostics",
      textExcerpt:
        "These services may collect information about page visits, device and browser characteristics, interactions, and approximate location. We use this information to understand site usage, verify changes, and diagnose usability issues."
    }
  ]);
  const topics = new Set(candidates.map((candidate) => candidate.topic));
