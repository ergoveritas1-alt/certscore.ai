...
drop trigger if exists set_pulse_requests_updated_at on public.pulse_requests;
create trigger set_pulse_requests_updated_at
...
drop trigger if exists set_pulse_domain_throttles_updated_at on public.pulse_domain_throttles;
create trigger set_pulse_domain_throttles_updated_at
before update on public.pulse_domain_throttles
for each row
execute function public.set_updated_at();
