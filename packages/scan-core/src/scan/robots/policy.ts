...
import { getCrawlerProductToken } from "../crawler-identity";
...
export function createRobotsPolicy(input: {
  ...
  return {
    ...
    crawlDelayMs(userAgent = getCrawlerProductToken()) {
      if (groups.length === 0) {
        return null;
      }
    ...
    allows(url: string, userAgent = getCrawlerProductToken()) {
      if (groups.length === 0) {
        return true;
      }
...
export function isUrlAllowedByRobots(url: string, robotsPolicy?: RobotsPolicy | null) {
  return robotsPolicy ? robotsPolicy.allows(url, getCrawlerProductToken()) : true;
}
