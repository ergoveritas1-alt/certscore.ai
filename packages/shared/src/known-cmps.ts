...
function collectSignalsForDefinition(definition: KnownCmpDefinition, input: KnownCmpDetectionInput) {
  ...
  for (const globalName of uniqueStrings(input.jsGlobals ?? [])) {
    if ((definition.globalNames ?? []).some((knownName) => {
      ...
      const normalizedKnownName = knownName.toLowerCase();
      if (
        GENERIC_CMP_PROTOCOL_GLOBALS.has(normalizedGlobalName) &&
        GENERIC_CMP_PROTOCOL_GLOBALS.has(normalizedKnownName)
      ) {
        return false;
      }
      return normalizedGlobalName === normalizedKnownName ||
        normalizedGlobalName.includes(normalizedKnownName);
    })) {
...
function detectionSpecificityScore(detection: KnownCmpDetection) {
  ...
  return detection.matchedSignals.reduce(
    (score, signal) => score + (sourceWeight[signal.source] ?? 1),
    0,
  );
}
