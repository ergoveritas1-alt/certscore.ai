 ...
 */
async function extractNetlogSockets(file,{signal,...options}={}){
 const limits={...DEFAULTS,...options};
 for(const key of Object.keys(DEFAULTS))if(!Number.isSafeInteger(limits[key])||limits[key]<=0||limits[key]>DEFAULTS[key])throw Error(`Invalid ${key}`);
 const stream=createReadStream(file,{highWaterMark:16*1024,signal});
 const decoder=new StringDecoder('utf8');let pending='',bytesRead=0,lineNumber=0,stage='header',tcp,socketType,endPhase,reason;
 const sockets=[];const ids=new Set();
 const fail=value=>{reason=value;};
 function line(raw){
  ...
  if(stage==='events'){
   ...
   try{
    ...
    const endpoint=text=>{if(typeof text!=='string')throw Error();const m=/^(?:127\.0\.0\.1|\[::1\]):([0-9]{1,5})$/.exec(text);const port=Number(m?.[1]);if(!m||port<1||port>65535)throw Error();return port;};
...
module.exports={extractNetlogSockets,NETLOG_MAX_SIZE_MB:16};
