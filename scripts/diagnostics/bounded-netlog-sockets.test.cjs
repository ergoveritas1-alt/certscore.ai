...
const log=events=>`${header}\n"events": [\n${events.map(e=>JSON.stringify(e)).join(',\n')}],\n"polledData": []\n}\n`;
async function read(text,options){const dir=await fs.mkdtemp(path.join(os.tmpdir(),'bounded-netlog-test-'));try{const file=path.join(dir,'log');await fs.writeFile(file,text);return await extractNetlogSockets(file,options);}finally{await fs.rm(dir,{recursive:true,force:true});}}
