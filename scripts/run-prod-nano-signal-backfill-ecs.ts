...
function getContainerCommand() {
  const raw = getEnv("OPS_NANO_SIGNAL_BACKFILL_COMMAND");
  return raw ? raw.split(/\s+/).filter(Boolean) : DEFAULT_CONTAINER_COMMAND;
}

function readInputJson(args: Map<string, string>) {
  const inline = args.get("input-json") ?? getEnv("OPS_NANO_SIGNAL_BACKFILL_INPUT_JSON");
  const inputPath = args.get("input") ?? getEnv("OPS_NANO_SIGNAL_BACKFILL_INPUT_FILE");
  if (inline && inputPath) {
  ...
  throw new Error("Missing backfill input. Pass --input <file> or --input-json '<json>'.");
}

function getBackfillEnvironment(args: Map<string, string>) {
  ...
  if (encodedInput.length > 100_000) {
    throw new Error("Backfill input is too large for ECS environment overrides. Reduce the scan set.");
  }
  return [
    { name: "OPS_NANO_SIGNAL_BACKFILL_INPUT_BASE64", value: encodedInput },
    {
      name: "OPS_NANO_SIGNAL_BACKFILL_RUN_ID",
      value: args.get("run-id") ?? getEnv("OPS_NANO_SIGNAL_BACKFILL_RUN_ID", `nano-signal-backfill-${Date.now()}`)
    }
  ];
}
...
async function main() {
  ...
  const taskDefinition = getEnv("OPS_NANO_SIGNAL_BACKFILL_TASK_DEFINITION", describedService?.taskDefinition ?? "");
  ...
  const containerName = getEnv("OPS_NANO_SIGNAL_BACKFILL_CONTAINER_NAME", container?.name ?? "");
  ...
  if (!containerName) {
    throw new Error(`Could not resolve backfill container name from task definition ${taskDefinition}.`);
  }
    ...
    containerOverrides: [
      {
        ...
        environment: getBackfillEnvironment(args),
        name: containerName
      }
    ]
  };
  ...
  if (runTaskPayload.failures?.length) {
    throw new Error(`ECS nano signal backfill task failed to start: ${JSON.stringify(runTaskPayload.failures)}`);
  }
  const taskArn = runTaskPayload.tasks?.[0]?.taskArn;
  if (!taskArn) {
    throw new Error("ECS nano signal backfill task did not return a task ARN.");
  }

  console.log(`Started AWS-side nano signal backfill task: ${taskArn}`);
  ...
  if (logGroup && logPrefix) {
    ...
    const logs = await aws([
      ...
      console.error(`Could not fetch backfill logs from ${logGroup}/${streamName}: ${error instanceof Error ? error.message : String(error)}`);
      return "";
    });
    if (logs) {
  ...
  const outputPath = args.get("output") ?? getEnv("OPS_NANO_SIGNAL_BACKFILL_OUTPUT_FILE");
  ...
  if (exitCode !== 0) {
    throw new Error(
      `AWS-side nano signal backfill failed with exit code ${exitCode}. Task status ${task?.lastStatus ?? "unknown"}; reason ${
