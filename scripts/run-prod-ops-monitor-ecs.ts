...
async function main() {
  ...
  const cluster = getRequiredEnv("OPS_AWS_MONITOR_ECS_CLUSTER", getEnv("AWS_VALIDATION_ECS_CLUSTER", getEnv("AWS_SCANNER_ECS_CLUSTER")));
  const service = getRequiredEnv("OPS_AWS_MONITOR_ECS_SERVICE", getEnv("AWS_VALIDATION_ECS_WORKER_SERVICE", "certscore-validation-worker"));
