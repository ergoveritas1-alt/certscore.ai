...
function getContainerCommand() {
  const raw = getEnv("OPS_PRECONSENT_RERUN_COMMAND");
  return raw ? raw.split(/\s+/).filter(Boolean) : DEFAULT_CONTAINER_COMMAND;
}

function readInputJson(args: Map<string, string>) {
  const inline = args.get("input-json") ?? getEnv("OPS_PRECONSENT_RERUN_INPUT_JSON");
  const inputPath = args.get("input") ?? getEnv("OPS_PRECONSENT_RERUN_INPUT_FILE");
  if (inline && inputPath) {
  ...
  throw new Error("Missing rerun input. Pass --input <file> or --input-json '<json>'.");
}

function getRerunEnvironment(args: Map<string, string>) {
  ...
  if (encodedInput.length > 100_000) {
    throw new Error("Rerun input is too large for ECS environment overrides.");
  }
  return [
    { name: "OPS_PRECONSENT_RERUN_INPUT_BASE64", value: encodedInput },
    { name: "OPS_PRECONSENT_RERUN_RUN_ID", value: args.get("run-id") ?? getEnv("OPS_PRECONSENT_RERUN_RUN_ID", `prod-preconsent-rerun-${Date.now()}`) }
  ];
}
...
async function main() {
  ...
  const taskDefinition = getEnv("OPS_PRECONSENT_RERUN_TASK_DEFINITION", describedService?.taskDefinition ?? "");
  ...
  const containerName = getEnv("OPS_PRECONSENT_RERUN_CONTAINER_NAME", container?.name ?? "");
  ...
  if (!containerName) {
    throw new Error(`Could not resolve rerun container name from task definition ${taskDefinition}.`);
  }
    ...
    containerOverrides: [
      {
        ...
        environment: getRerunEnvironment(args),
        name: containerName
      }
    ]
  };
  ...
  if (runTaskPayload.failures?.length) {
    throw new Error(`ECS production preconsent rerun task failed to start: ${JSON.stringify(runTaskPayload.failures)}`);
  }
  const taskArn = runTaskPayload.tasks?.[0]?.taskArn;
  if (!taskArn) {
    throw new Error("ECS production preconsent rerun task did not return a task ARN.");
  }

  console.log(`Started AWS-side preconsent rerun task: ${taskArn}`);
  ...
  if (logGroup && logPrefix) {
    ...
    const logs = await aws([
      ...
      console.error(`Could not fetch rerun logs from ${logGroup}/${streamName}: ${error instanceof Error ? error.message : String(error)}`);
      return "";
    });
    if (logs) {
  ...
  const outputPath = args.get("output") ?? getEnv("OPS_PRECONSENT_RERUN_OUTPUT_FILE");
  ...
  if (exitCode !== 0) {
    throw new Error(
      `AWS-side preconsent rerun failed with exit code ${exitCode}. Task status ${task?.lastStatus ?? "unknown"}; reason ${
