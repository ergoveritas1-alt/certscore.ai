#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEB_ENV_FILE="$ROOT_DIR/apps/web/.env.local"
WS01_ROOT="${WS01_ROOT:-$(cd "$ROOT_DIR/.." && pwd)/WS01}"
WS01_SCANNER_DIR="$WS01_ROOT/apps/scanner"

if [[ ! -d "$WS01_SCANNER_DIR" ]]; then
  echo "[run-ws01-scanner-dev] expected WS01 scanner workspace at $WS01_SCANNER_DIR" >&2
  exit 1
fi

if [[ ! -f "$WEB_ENV_FILE" ]]; then
  echo "[run-ws01-scanner-dev] missing $WEB_ENV_FILE" >&2
  exit 1
fi

set -a
source "$WEB_ENV_FILE"
set +a

required_vars=(
  NEXT_PUBLIC_SUPABASE_URL
  NEXT_PUBLIC_SUPABASE_ANON_KEY
  SUPABASE_SERVICE_ROLE_KEY
)

for var_name in "${required_vars[@]}"; do
  if [[ -z "${!var_name:-}" ]]; then
    echo "[run-ws01-scanner-dev] missing $var_name in $WEB_ENV_FILE" >&2
    exit 1
  fi
done

export WORKER_CONCURRENCY="${WORKER_CONCURRENCY:-3}"
export SCANNER_POLL_INTERVAL_MS="${SCANNER_POLL_INTERVAL_MS:-500}"
export SCANNER_STALE_SCAN_THRESHOLD_MS="${SCANNER_STALE_SCAN_THRESHOLD_MS:-3600000}"
export SCANNER_START_JITTER_MIN_MS="${SCANNER_START_JITTER_MIN_MS:-0}"
export SCANNER_START_JITTER_MAX_MS="${SCANNER_START_JITTER_MAX_MS:-500}"
export SCANNER_CRAWLER_NAME="${SCANNER_CRAWLER_NAME:-SignalScannerBot}"
export SCANNER_CRAWLER_PUBLIC_URL="${SCANNER_CRAWLER_PUBLIC_URL:-https://mycrawler.cloud}"
export SCANNER_HYBRID_AUTO_ENABLED="${SCANNER_HYBRID_AUTO_ENABLED:-1}"
export SCANNER_HYBRID_CDP_ROTATE_EVERY="${SCANNER_HYBRID_CDP_ROTATE_EVERY:-6}"
export SCANNER_HYBRID_CDP_BASE_PORT="${SCANNER_HYBRID_CDP_BASE_PORT:-9322}"
export LLM_ENRICHMENT_ENABLED="0"
export SURFACE_RECOVERY_LLM_ENABLED="0"
export POLICY_ENRICHMENT_MOCK_LLM="0"
export SURFACE_RECOVERY_LLM_MOCK="0"
export PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-$HOME/Library/Caches/ms-playwright}"
export SUPABASE_STORAGE_BUCKET="${SUPABASE_STORAGE_BUCKET:-scan-reports}"
export SUPABASE_STORAGE_BUCKET_SCREENSHOTS="${SUPABASE_STORAGE_BUCKET_SCREENSHOTS:-scan-screenshots}"
export SUPABASE_STORAGE_BUCKET_ARTIFACTS="${SUPABASE_STORAGE_BUCKET_ARTIFACTS:-scan-artifacts}"
export WS01_SCAN_CORE_PATH="${WS01_SCAN_CORE_PATH:-$WS01_ROOT/packages/scan-core/src/index.ts}"

if [[ ! -f "$WS01_SCAN_CORE_PATH" ]]; then
  echo "[run-ws01-scanner-dev] expected WS01 scan-core entrypoint at $WS01_SCAN_CORE_PATH" >&2
  exit 1
fi

echo "[run-ws01-scanner-dev] starting WS01 scanner against ${NEXT_PUBLIC_SUPABASE_URL}"

cd "$WS01_SCANNER_DIR"
if command -v node >/dev/null 2>&1; then
  NODE_MAJOR_VERSION="$(node -p 'process.versions.node.split(".")[0]')"
else
  NODE_MAJOR_VERSION=""
fi

if [[ -n "$NODE_MAJOR_VERSION" && "$NODE_MAJOR_VERSION" -ge 22 ]]; then
  exec node --enable-source-maps --import tsx \
    --watch-path=./src \
    --watch-path=../../packages/scan-core/src \
    ./src/index.ts
fi

exec npx -y node@22 --enable-source-maps --import tsx \
  --watch-path=./src \
  --watch-path=../../packages/scan-core/src \
  ./src/index.ts
