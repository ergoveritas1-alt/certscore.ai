...
async function waitForTerminalScan(created) {
  ...
  while (true) {
    ...
    if (!scanId && !created.jobId) {
      throw new Error(`MCP scan is non-terminal (${status}) but certscore_scan_site returned no scanId or jobId.`);
    }
    ...
    console.log(`certscore_get_scan_status status=${current.status ?? "unknown"} phase=${current.phase ?? "unknown"} scanId=${getScanId(current) ?? "none"}`);
...
function verifyScanAndFindings(scan, findings, expectedScanId) {
  ...
  if (!Array.isArray(findings.findings)) throw new Error("MCP certscore_list_findings did not include a findings array.");
...
try {
  ...
  console.log(`certscore_scan_site status=${created.status} jobId=${created.jobId ?? "none"} scanId=${created.scanId ?? "none"}`);

  if (!getScanId(created) && !created.jobId) {
    throw new Error("certscore_scan_site returned neither scanId nor jobId.");
  }
