...
const openAiSkill = read("integrations/openai/certscore-website-privacy-preflight/skills/website-privacy-preflight/SKILL.md");
assert.match(submissionPacket, /OpenAI package is `2\.0\.0`/);
assert.doesNotMatch(submissionPacket, /OpenAI package is `1\.0\.0`/);
for (const requiredTerm of ["gpcResponse", "postAcceptObservation", "postRefusalObservation"]) {
  assert.match(openAiSkill, new RegExp(requiredTerm), `OpenAI skill omits ${requiredTerm}.`);
}
...
assert.match(read("apps/web/public/llms-full.txt"), /current hosted MCP version is `0\.2\.18`/);
